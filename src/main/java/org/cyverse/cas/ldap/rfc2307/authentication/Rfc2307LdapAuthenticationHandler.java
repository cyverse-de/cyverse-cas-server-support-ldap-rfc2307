...
@Setter
@Getter
public class Rfc2307LdapAuthenticationHandler extends AbstractUsernamePasswordAuthenticationHandler {
    private static final Logger LOGGER
            = LoggerFactory.getLogger(Rfc2307LdapAuthenticationHandler.class);
     ...
     */
    public Rfc2307LdapAuthenticationHandler(final String name, final ServicesManager servicesManager,
    ...
    protected Map<String, List<Object>> collectAttributesForLdapEntry(final LdapEntry ldapEntry, final String username) {
        ...
        try {
            attributeMap.put("entitlement", new ArrayList<>(groupMembershipResolver.resolve(ldapEntry)));
        } catch (LdapException e) {
            LOGGER.warn("error encountered during group membership resolution: {}", e.getMessage());
        }
